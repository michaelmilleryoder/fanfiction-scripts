...
def add_character_features_fandom(params):
    ...
    # Sample from the fandom
    data = data.sample(8400)
...
class CharacterFeaturesDataset:
    ...
    def match_pipeline_chars(self):
        ...
        self.metadata_parts = {fandom: part for fandom, part in zip(
            self.fandoms, parts)}

    def annotate_relationships(self):
        """ Annotate relationships straight/queer based on character gender 
            inferred from pronouns.
            Not used (incorporated into match_pipeline_chars)
        """
        #with Pool(len(self.fandoms)) as p:
        #    list(tqdm(p.imap(self.annotate_relationships_fandom, self.fandoms), 
        #        ncols=70, total=len(self.fandoms)))
        # for debugging
        list(tqdm(map(self.annotate_relationships_fandom, self.fandoms), 
                ncols=70, total=len(self.fandoms)))

    def annotate_relationships_fandom(self, fandom):
        data = self.metadata_parts[fandom]
        data['relationship_type'] = [annotate_relationship_gender(fic_id, rel, 
            self.coref_dirpaths[fandom]) for fic_id, rel, fandom in list(zip(
            data.index, data.coref_char_matches, data.fandom))]

    def add_character_features(self, assertions=True, quotes=False):
        """ Add in assertions and quotes for characters """
        params = [(self.metadata_parts[fandom], self.charfeats_dirpaths[fandom]) for \
            fandom in self.fandoms]
        with Pool(len(self.fandoms)) as p:
    ...
    def transform(self):
        ...
        #print('Annotating relationships...')
    ...
    def save(self):
        ...
        self.data = pd.concat(self.metadata_parts.values())
