...
min_df <- 100 # minimum document frequency for words for assertions
...
cat('Processing data...\n')
processed <- textProcessor(data$assertion_features, stem=do_stem, metadata=data,
		verbose=TRUE)
...
outpath <- sprintf('/projects/fanfiction_lgbtq/models/assertions_stm_%s_%dtopics_%dit_%dmindf.rds', covariates, num_topics, max_iters, min_df)
